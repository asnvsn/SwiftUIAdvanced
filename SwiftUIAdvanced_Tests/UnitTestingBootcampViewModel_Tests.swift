...
// Naming Structure: test_[struct or class]_[variable of function]_[expected result]
...
final class UnitTestingBootcampViewModel_Tests: XCTestCase {
    ...
    func test_UnitTestingBootcampViewModel_selectedItem_shouldBeNillWhenSelectingInvalidStartAsNil() {
        // Given
        let vm = UnitTestingBootcampViewModel(isPremium: Bool.random())
    ...
    func test_UnitTestingBootcampViewModel_selectedItem_shouldBeSelected_stress() {
        ...
        let randomArray = itemsArray.randomElement() ?? ""
        XCTAssertFalse(randomArray.isEmpty)
        vm.selectItem(item: randomArray)
        
        //Then
        XCTAssertNotNil(vm.selectedItem)
        XCTAssertEqual(vm.selectedItem, randomArray)
    }
    ...
    func test_UnitTestingBootcampViewModel_saveItem_shouldThrowError_noData() {
        ...
        do {
            ...
            let returnedError = error as? UnitTestingBootcampViewModel.DataError
            XCTAssertEqual(returnedError, UnitTestingBootcampViewModel.DataError.notData)
        }
    }
    ...
    func test_UnitTestingBootcampViewModel_downloadWithCombine_shouldReturnItems() {
        ...
        // When
        let expectation = XCTestExpectation(description: "Should return items after second")
